    ...
    if allowed:
        ...
        if get_setting("auto_scan_prompt", "1") == "1":
            ...
            if wants_scan:
                ...
                log_activity("virus_scan_started", f"Virus scan started on {drive_letter}")
                ...
                if scan_result.get("infected_count", 0) > 0:
                    log_activity("threats_found", f"Virus scan of {drive_letter}: {scan_result['infected_count']} threats found")
                else:
                    log_activity("virus_scan_complete", f"Virus scan of {drive_letter} complete: clean")
            else:
...
def start_watching():
    ...
    log_activity("protection_started", "USB protection watcher started")
